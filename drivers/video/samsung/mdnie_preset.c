			...
			const unsigned short source_standard[],	const unsigned short source_natural[])
{
	...
	// print debug info
	printk("Kernel: mdnie control device updated for mode: %d\n", m->mode);
}
					...
					const char *buf, size_t count)
{
	...
	if ((val >= 0) && (val <= 1))
	{
		...
		if (mdnie_preset == 0) // original Samsung mdnie settings
		{
			...
			// print debug info
			printk("Kernel: mdnie preset set to original\n");
		...
		if (mdnie_preset == 1) // Hardcore speedmod mdnie settings
		{
			...
			// print debug info
			printk("Kernel: mdnie preset set to Hardcore speedmod\n");
		}
...
static int mdnie_preset_init(void)
{
	...
	if (sysfs_create_group(&mdnie_preset_control_device.this_device->kobj,
		...
		printk("Kernel: failed to create sys fs object.\n");
		return 0;
	}

	// Print debug info
	printk("Kernel: mdnie control device started\n");
...
static void mdnie_preset_exit(void)
{
	...
	// Print debug info
	printk("Kernel: mdnie control device stopped\n");
}
